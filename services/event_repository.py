...
def init_db():
    """Inicializa o banco de dados com a tabela de eventos."""
...
def save_event(event_id: str, label: str, confidence: float, image_path: str):
    """Salva um evento detectado no banco de dados."""
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""
...
def list_events(limit: int = 50) -> List[Dict]:
    """Lista os eventos mais recentes do banco de dados."""
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""
...
def count_events() -> int:
    """Conta o número total de eventos no banco de dados."""
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
